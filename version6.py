...
def find_shortest_path_from_vertex_to_set(state, source: int, target_set: set[int]) -> list[int]:
    ...
    while len(queue) != 0:
        current = queue.pop()
        seen[current] = True
        write_log('log.txt', f'current = {current}')
        if current in target_set:
            break

        for neighbour in state.map.get_adjacent_to(current):
            write_log('log.txt', f'neighbour = {neighbour}')
            if neighbour not in seen:
                seen[neighbour] = True
                parent[neighbour] = current
                queue.appendleft(neighbour)

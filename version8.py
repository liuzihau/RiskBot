...
class Bot:
    ...
    def put_troops_on_attack_territory(self, src, tgt, max_troops, min_troops):
        ...
        if src in potential_border:
            ...
            write_log(self.clock, "AfterAttack", f"trying occupy {DOOR[self.plan['name']]}, and {src} is door, put {final_troops} for protecting")
            return final_troops
        else:
            write_log(self.clock, "AfterAttack", f"trying occupy {DOOR[self.plan['name']]}, and {src} is not door, put 1")
            return max_troops - 1
    ...
    def fortify_troops(self):
        ...
        if src is not None and src != tgt:
            write_log(self.clock, "Fortify", f"move from {src} to {set([tgt])} within our territories group {group} (our territoreis={self.state.get_territories_owned_by(self.id_me)})")
